...
- **Founder tools** (`#admin`, or the discreet link on How Tariga works) — **two tabs, two jobs (§28.2):**
  ...
  - **Content manager (§26):** searchable/filterable tables for scenarios (domain/tier/status chips + full-text search), vocabulary, and call sequences — inline editing stored as local overrides applied on load; audit-queue flags land here for action; quick-add founder content (labeled pending-review); contributors + vouch codes; pending queue with force-approve/reject + bulk actions (bypasses vote weighting, for urgent fixes); analytics glance (event counts + most-looked-up missing words); full **JSON/CSV export** of the content database; and a **Show / hide table (visibility controls)** — hide any whole mode, guided scenario, coach scenario, or vocabulary item from learners without deleting it (hidden things vanish from the sidebar, tab bar, home cards, decks and the domain map; a "hidden" badge keeps them manageable in the content tables and one tap brings them back; demo mode can still preview hidden screens). **Publish to everyone:** all content-manager work (edits, quick-adds, deletions, hide/show) starts as a draft in the founder's browser; one button — guarded by the Worker's STATS_KEY secret, typed each time and never stored — pushes the overrides blob to the Worker's D1, and **every visitor's app pulls it automatically at load** (shipped data → published overrides → the founder's own unpublished drafts on top). The Worker URL is baked into the app config, so visitors get the coach, event logging and published content with zero setup — the API key still exists only as a Cloudflare secret.

## 8b-1. The tariga-handoff home experience (dark + light)

The default (warm) home was rebuilt 1:1 from the `tariga-handoff/` React mockups, ported into the no-build architecture and wired to real data:
- **Intro gate** (once per day): breathing WebGL shader orb (the handoff's exact GLSL, warm gold/copper fbm plasma with fresnel rim), السلام عليكم greeting, romanisation, a glass input row (gradient mic + "Say it →") that actually checks the learner's attempt on the shared compare engine, skip link.
- **Dashboard**: ambient triple-glow + fractal-noise background, طريقة wordmark nav with a **dark/light theme toggle** (full parchment light palette, persisted), giant Arabic greeting, glass **continue card** (real `nextFocus()` + WebGL orb + play), **stats ring** (real coached %, animated draw-in) with three live stat rows, four **module tiles** with 3D mouse tilt + sheen, and **شجرتك The Tree**: a tapered-bark SVG tree (filled lit-cylinder gradient shapes, never stroked lines) whose five branches carry the real curriculum's moments as glow/pulse/hollow nodes, behind a canvas of 50 falling ember particles. Tapping a branch **zooms the SVG** (per-domain transform targets), fades siblings to 20%, reveals per-node labels and glass callout cards with a real "Start Session", plus an **"Explore this branch in 3D"** CTA.
- **Branch detail**: a full-screen **CSS 3D tree** (no WebGL) — six-panel tapered trunk, 14 procedural decorative branches, per-moment branch arms, amber path lines lit only along real progress, 18 float-up particles, and **drag-to-rotate** (±25°/±45°) with node spheres that **counter-rotate to always face the camera**. Nodes are the selected domain's real moments (Arabic from verified targets); selecting one flies the camera in (scale 1.4) and slides up a detail panel — Arabic + English title, real mastery %, shimmer progress bar, Start/Review Lesson straight into the guided session or call. Orbit-style header: طريقة wordmark + a live XP pill.
- **Language display toggle** (ع / EN / a·b chips, on the tree card and the 3D branch header): show any mix of Arabic script, English, and transliteration on tree labels, callouts and the 3D node pills — persisted per browser, never allowed to go fully empty.
- **Three 3D branch views** behind a Tree / Orbit / Helix switcher: the bark parallax tree (drag), an Orbit.tsx-style climbing spine of glowing spheres with golden progress segments and slow auto-orbit, and a helix that spirals the moments around the trunk — all pure CSS 3D, all sharing the same real data, selection panel and language toggle.
